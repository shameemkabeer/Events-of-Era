...
def pay():
    ...
    qry = "INSERT INTO payment (payment_id, booking_id, amount, payment_date) VALUES (NULL, %s, %s, CURDATE())"
...
def view_ratings():
    ...
    qry1="SELECT feedback_id,rating_id,first_name,last_name,feedback,feedback_date,rating_no FROM feedback inner join rating using(customer_id) inner join customer1 using(customer_id)"
    result = db.select(qry1)
...
def admgetcustomevent():
    ...
    qry3 = "SELECT ce.custom_event_id,ce.customer_id,ce.event_name,ce.budget,ce.place,ce.no_of_persons,ce.event_date,ce.event_status,cf.custom_event_id,f.food_id,f.food_name,f.quantity,f.serving_type FROM custom_events AS ce INNER JOIN customeventfoods AS cf ON ce.custom_event_id = cf.custom_event_id INNER JOIN food AS f ON cf.food_id = f.food_id"
